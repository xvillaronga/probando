...
//$id = $_POST['usuario'];
	...
	$result = $conexion->query("SELECT DE.provincia as Provincia, COUNT(DE.provincia) as N_delegados ,COUNT(IF(DE.sexo='M',DE.sexo,NULL)) as Mujeres,FORMAT(((COUNT(IF(DE.sexo='M',DE.sexo,NULL))/ COUNT(DE.provincia))*100),2) as PorcentajeMujeres ,COUNT(IF(DE.sexo='H',DE.sexo,NULL)) as Hombres,FORMAT(((COUNT(IF(DE.sexo='H' ,DE.sexo,NULL))/ COUNT(DE.provincia))*100),2) as PorcentajeHombres  FROM delegados DE GROUP BY DE.provincia");
